...
export class OfflineEstimateTest {
  ...
  private static async testEstimateGeneration(): Promise<void> {
    ...
    const estimate = await offlineEstimateService.generateEstimate(testEstimateData);
    
    if (!estimate.referenceNumber || estimate.breakdown.total === 0) {
      throw new Error('Estimate generation failed');
    }
    
    console.log('✅ Estimate generation works', estimate.referenceNumber, estimate.breakdown.total);
  }
  ...
  private static async testEstimateStorage(): Promise<void> {
    ...
    const retrievedEstimate = await offlineEstimateService.getEstimateById(firstEstimate.id);
...
if (process.env.NODE_ENV === 'development') {
  console.log('🔧 Development mode detected - offline estimate tests available');
  console.log('Run OfflineEstimateTest.runAllTests() in console to test offline functionality');
}
