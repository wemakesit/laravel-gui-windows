...
export default function ExtrasForm({ windowData, extras, onSave, onCancel }) {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            <div>
                <h3 className="text-lg font-medium text-gray-900 mb-4">Available Extras</h3>
                        ...
                        {extras.extras.map((extra, index) => (
                            <div key={index} className="flex items-center">
                                <input
                                    ...
                                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleExtraToggle(extra)}
                                    className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                                />
                                <label htmlFor={`extra-${index}`} className="ml-3 flex justify-between w-full">
                                    <span className="text-sm font-medium text-gray-700">{extra.Name}</span>
                                    <span className="text-sm text-gray-500">£{extra.Cost.toFixed(2)}</span>
                                </label>
                            </div>
                        ))}
                    </div>
                    ...
                    <ul className="space-y-2">
                        {formData.extras.map((extra, index) => (
                            <li key={index} className="flex justify-between">
                                ...
                                <span className="text-sm font-medium text-gray-900">£{extra.cost.toFixed(2)}</span>
                            </li>
                        ))}
                    </ul>
                    <div className="mt-4 pt-4 border-t border-gray-200 flex justify-between">
                        <span className="text-sm font-medium text-gray-700">Total Extras Cost:</span>
                        <span className="text-sm font-medium text-gray-900">
                            £{formData.extras.reduce((total, extra) => total + extra.cost, 0).toFixed(2)}
                        </span>
                    </div>
                </div>
            )}
