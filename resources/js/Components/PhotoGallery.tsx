...
export default function PhotoGallery({
  ...
  const loadPhotos = async () => {
    try {
      ...
      const filteredPhotos = windowIndex !== undefined 
            ...
            <div key={photo.id} className="relative group">
              ...
              <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-50 transition-opacity rounded-lg flex items-end">
                <div className="p-2 text-white opacity-0 group-hover:opacity-100 transition-opacity w-full">
                  {showWindowLabels && (
                    <p className="text-xs font-medium">Window {photo.windowIndex + 1}</p>
                  )}
                  <p className="text-xs">{formatFileSize(photo.metadata.size)}</p>
                </div>
              </div>
              ...
              {/* Sync Status */}
              {!photo.synced && (
                <div className="absolute top-2 left-2 p-1 bg-yellow-500 text-white rounded-full" title="Not synced">
                  <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="p-4 border-b border-gray-200 flex justify-between items-center">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {formatDate(selectedPhoto.timestamp)} • {formatFileSize(selectedPhoto.metadata.size)}
                </p>
              </div>
              <button
