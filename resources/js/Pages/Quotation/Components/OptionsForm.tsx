...
export default function OptionsForm({ windowData, options, onSave, onCancel }) {
  ...
  const [selectedOptions, setSelectedOptions] = useState(() => {
    ...
    if (
      Array.isArray(initialWindowData.options) &&
      initialWindowData.options.length > 0
    ) {
  ...
  const handleOptionToggle = optionId => {
    ...
    const optionsArray = Object.keys(newSelectedOptions).map(id =>
      parseInt(id, 10)
    );

    // If only one option is selected, store it as a number, otherwise as an array
    const optionsValue =
      optionsArray.length === 1 ? optionsArray[0] : optionsArray;
    ...
    <form onSubmit={handleSubmit} className='space-y-6'>
      <div>
        ...
        <p className='text-sm text-gray-500 mb-4'>
          Select which options this window should be included in. A window can
          be part of multiple options.
        </p>
        ...
        <div className='mt-6 bg-gray-50 p-4 rounded-md'>
          ...
          <ul className='space-y-2'>
            {Object.keys(selectedOptions).map(optionId => {
              const option = effectiveOptions.options.find(
