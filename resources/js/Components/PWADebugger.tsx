...
export default function PWADebugger() {
  ...
  const checkPWAStatus = async () => {
    ...
    try {
      ...
      pwaStatus.installation.platform = (navigator as any).userAgentData?.platform || navigator.platform;
      ...
      pwaStatus.storage.pouchDB = typeof (window as any).PouchDB !== 'undefined';
  ...
  const StatusIndicator = ({ status: isGood, label }: { status: boolean; label: string }) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-gray-50 p-4 rounded-lg">
          ...
          <div className="space-y-2 text-sm">
            <StatusIndicator status={status.serviceWorker.registered} label="Registered" />
            <StatusIndicator status={status.serviceWorker.active} label="Active" />
            {status.serviceWorker.scope && (
              <p className="text-gray-600 text-xs">Scope: {status.serviceWorker.scope}</p>
            )}
            {status.serviceWorker.state && (
              <p className="text-gray-600 text-xs">State: {status.serviceWorker.state}</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-gray-50 p-4 rounded-lg">
          ...
          <div className="space-y-2 text-sm">
            <StatusIndicator status={status.manifest.available} label="Available" />
            {status.manifest.name && (
              <p className="text-gray-600 text-xs">Name: {status.manifest.name}</p>
            )}
            {status.manifest.display && (
              <p className="text-gray-600 text-xs">Display: {status.manifest.display}</p>
            )}
            {status.manifest.icons && (
              <p className="text-gray-600 text-xs">Icons: {status.manifest.icons}</p>
            )}
            {status.manifest.error && (
              <p className="text-red-600 text-xs">Error: {status.manifest.error}</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-gray-50 p-4 rounded-lg">
          ...
          <div className="space-y-2 text-sm">
            <StatusIndicator status={status.installation.standalone} label="Standalone Mode" />
            <StatusIndicator status={status.installation.installable} label="Installable" />
            {status.installation.platform && (
              <p className="text-gray-600 text-xs">Platform: {status.installation.platform}</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-gray-50 p-4 rounded-lg">
          ...
          <div className="space-y-2 text-sm">
            <StatusIndicator status={status.storage.localStorage} label="Local Storage" />
            <StatusIndicator status={status.storage.indexedDB} label="IndexedDB" />
            <StatusIndicator status={status.storage.pouchDB} label="PouchDB" />
            {status.storage.estimates !== undefined && (
              <p className="text-gray-600 text-xs">Estimates: {status.storage.estimates}</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-gray-50 p-4 rounded-lg">
          ...
          <div className="space-y-2 text-sm">
            ...
            {status.network.connection && (
              <p className="text-gray-600 text-xs">Type: {status.network.connection}</p>
            )}
            {status.network.effectiveType && (
              <p className="text-gray-600 text-xs">Speed: {status.network.effectiveType}</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-gray-50 p-4 rounded-lg">
          ...
          <div className="space-y-2 text-sm">
            <StatusIndicator status={status.cache.available} label="Available" />
            {status.cache.entries && (
              <p className="text-gray-600 text-xs">Entries: {status.cache.entries}</p>
            )}
            {status.cache.size && (
