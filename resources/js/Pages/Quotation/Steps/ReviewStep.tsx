                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <p className="text-base text-gray-900">
                            {formData.customer_details.title ? `${formData.customer_details.title} ` : ''}
                            {formData.customer_details.first_name} {formData.customer_details.last_name}
                        </p>
                    </div>
                    <div>
                        <p className="text-sm font-medium text-gray-500">Email</p>
                        <p className="text-base text-gray-900">{formData.customer_details.email}</p>
                    </div>
                    <div>
                        <p className="text-sm font-medium text-gray-500">Phone</p>
                        <p className="text-base text-gray-900">{formData.customer_details.phone}</p>
                    </div>
                    <div className="md:col-span-2">
                        <p className="text-sm font-medium text-gray-500">Address</p>
                        <p className="text-base text-gray-900">{formData.customer_details.address}</p>
                    </div>
                    {formData.customer_details.additional_info && (
                        <div className="md:col-span-2">
                            <p className="text-sm font-medium text-gray-500">Additional Information</p>
                            <p className="text-base text-gray-900">{formData.customer_details.additional_info}</p>
                        </div>
                    )}
                </div>
            </div>
                    ...
                    <div className="space-y-6">
                            ...
                            <div key={index} className="border border-gray-200 rounded-md p-4">
                                ...
                                <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-900">{window.glass_specification || 'Not specified'}</p>
                                    </div>
                                    <div>
                                        <p className="text-xs font-medium text-gray-500">Paint Finish</p>
                                        <p className="text-sm text-gray-900">{window.paint_finish || 'Not specified'}</p>
                                    </div>
                                    <div>
                                        <p className="text-xs font-medium text-gray-500">Hardware Finish</p>
                                    ...
                                    <div className="mt-4">
                                        ...
                                        <ul className="mt-1 space-y-1">
                                            {window.extras.map((extra, extraIndex) => (
                                                <li key={extraIndex} className="text-sm flex justify-between">
                                                    <span className="text-gray-600">{extra.name}</span>
                                                    <span className="text-gray-900">£{extra.cost.toFixed(2)}</span>
                                                </li>
                                            ))}
                                        </ul>
                                    </div>
                                ...
                                {window.additional_info && (
                                    <div className="mt-4">
                                        ...
                                        <p className="text-sm text-gray-600">{window.additional_info}</p>
                                    </div>
                                )}
                            </div>
                        ))}
                        ...
                        <div className="mt-6 border-t border-gray-200 pt-4">
                            ...
                            <div className="flex justify-between text-sm mt-2">
                                <span className="font-medium text-gray-700">VAT ({(pdfTextConfig?.formats?.vat_rate || 0.2) * 100}%):</span>
                                <span className="text-gray-900">£{calculateVAT().toFixed(2)}</span>
                            </div>
                    ...
                    {defaultCaveats.map((caveat, index) => (
                        <div key={index} className="flex items-start">
                            <input
                                ...
                                onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleCaveatToggle(caveat)}
                                className="h-4 w-4 mt-1 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                            />
                            <label htmlFor={`caveat-${index}`} className="ml-3 text-sm text-gray-700">
                                {caveat}
                            </label>
                        </div>
                    ))}
