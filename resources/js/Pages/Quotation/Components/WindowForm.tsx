...
export default function WindowForm({
  ...
  const getWindowTypesArray = () => {
    ...
    if (windowTypes.window_types && !Array.isArray(windowTypes.window_types)) {
      console.warn(
        'WindowForm: windowTypes.window_types is not an array',
    ...
    if (typeof windowTypes === 'object' && !('window_types' in windowTypes)) {
      console.warn(
        'WindowForm: windowTypes object does not have window_types property',
        windowTypes
      );

      // Try to extract any array that might be in the object
      const possibleArrays = Object.values(windowTypes).filter(value =>
        Array.isArray(value)
      );
    ...
    console.warn(
      'WindowForm: Could not extract window types from provided data',
      windowTypes
    );
    return [];
  };
