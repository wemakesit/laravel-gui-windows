...
export default function Dashboard({
  ...
  const handleSyncToServer = async () => {
    try {
      ...
      // Get all estimates and sync them
      const estimates = await watermelonDBService.getAllEstimates();
      let syncedCount = 0;
      let failedCount = 0;

      for (const estimate of estimates) {
        const result = await watermelonDBService.syncEstimateToServer(estimate.id);
        if (result.success) {
          syncedCount++;
        } else {
          failedCount++;
          console.error(`Failed to sync estimate ${estimate.id}:`, result.error);
        }
      }

      alert(`Sync completed! Synced: ${syncedCount}, Failed: ${failedCount}`);

      // Reload dashboard data to show updated sync status
  ...
  const handleLoadFromServer = async () => {
    try {
      console.log('📥 Loading estimates from server...');

      const result = await watermelonDBService.loadEstimatesFromServer();
      if (result.success) {
        alert(`Loaded ${result.count} estimates from server`);
        await loadDashboardData();
      } else {
          ...
          <div className='flex items-center space-x-4'>
            ...
            <div className='flex space-x-2'>
              <button
                ...
                title='Sync local estimates to server'
              >
                📤 Sync to Server
              </button>
              <button
                ...
                title='Load estimates from server'
              >
                📥 Load from Server
              </button>
            </div>
