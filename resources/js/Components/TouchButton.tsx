...
const TouchButton = forwardRef<HTMLButtonElement, TouchButtonProps>(({
  ...
  const { capabilities, getTouchClasses, hapticFeedback: triggerHaptic } = useTouch();

  const baseClasses = 'btn-touch focus-visible-touch transition-all duration-200 font-medium rounded-lg border-0 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed';
