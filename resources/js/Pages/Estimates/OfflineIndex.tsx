...
export default function OfflineIndex() {
  ...
  const [filteredEstimates, setFilteredEstimates] = useState<CompletedEstimate[]>([]);
  ...
  const applyFilters = () => {
    ...
    if (filters.search) {
      ...
      filtered = filtered.filter(estimate => 
        ...
        `${estimate.customerDetails.first_name} ${estimate.customerDetails.last_name}`.toLowerCase().includes(searchLower) ||
    ...
    if (filters.dateRange !== 'all') {
      ...
      filtered = filtered.filter(estimate => 
        new Date(estimate.createdAt) >= filterDate
      );
    }
  ...
  const handleBulkDelete = async () => {
    ...
    if (!confirm(`Are you sure you want to delete ${selectedEstimates.length} estimate(s)? This action cannot be undone.`)) {
      return;
    }
    ...
    try {
      ...
      await loadEstimates();
      showNotification('success', `${selectedEstimates.length} estimate(s) deleted successfully`);
    } catch (error) {
          ...
          <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div className="p-6">
              ...
              <div className="flex justify-between items-center mb-6">
                <div>
                  <h1 className="text-2xl font-semibold text-gray-900">All Estimates</h1>
                      ...
                      <tr>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                          <input
                            ...
                            onChange={(e) => {
                              if (e.target.checked) {
                                setSelectedEstimates(filteredEstimates.map(est => est.id));
                              } else {
                                setSelectedEstimates([]);
                              }
                            }}
                      ...
                      {filteredEstimates.map((estimate) => (
                        <tr key={estimate.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            {estimate.metadata.windowCount} ({estimate.metadata.totalItems} items)
                          </td>
