...
export default function OfflineShow({ estimateId }: OfflineShowProps) {
  ...
  const loadEstimate = async () => {
    try {
      setLoading(true);
      const loadedEstimate = await offlineEstimateService.getEstimateById(estimateId);
      if (!loadedEstimate) {
      ...
      <>
        ...
        <div className="min-h-screen flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-6">The estimate you're looking for doesn't exist or has been deleted.</p>
            <Link
              href="/estimates"
    ...
    <>
      ...
      <div className="py-12">
        <div className="max-w-4xl mx-auto sm:px-6 lg:px-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-900">
                    {estimate.customerDetails.title} {estimate.customerDetails.first_name} {estimate.customerDetails.last_name}
                  </p>
                  <p className="text-sm text-gray-500">{estimate.customerDetails.email}</p>
                  <p className="text-sm text-gray-500">{estimate.customerDetails.phone}</p>
                </div>
                <div>
                  <h3 className="text-sm font-medium text-gray-500">Windows</h3>
                  <p className="mt-1 text-sm text-gray-900">
                    {estimate.metadata.windowCount} windows ({estimate.metadata.totalItems} total items)
                  </p>
                </div>
                <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="mt-2 space-y-1">
                    ...
                    <p className="text-sm text-gray-900">{estimate.customerDetails.email}</p>
                    <p className="text-sm text-gray-900">{estimate.customerDetails.phone}</p>
                  </div>
                </div>
                <div>
          ...
          <div className="bg-white shadow-sm rounded-lg mb-6">
            <div className="px-6 py-4 border-b border-gray-200">
              <h2 className="text-lg font-medium text-gray-900">Windows & Configuration</h2>
            </div>
            <div className="px-6 py-4">
                ...
                {estimate.windows.map((window, index) => (
                  <div key={window.id} className="border border-gray-200 rounded-lg p-4">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <h4 className="text-base font-medium text-gray-900">{window.room}</h4>
                        <p className="text-sm text-gray-600">{window.type}</p>
                        <p className="text-sm text-gray-500">
                          Quantity: {window.quantity} | Options: {window.options.join(', ')}
                        </p>
                      </div>
                      ...
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          ...
                          <span className="text-gray-900">
                            {formatCurrency(window.basePrice)} × {window.quantity} = {formatCurrency(window.basePrice * window.quantity)}
                          </span>
                        </div>
                        ...
                        {window.extras.length > 0 && (
                          <div>
                            ...
                            {window.extras.map((extra, extraIndex) => (
                              <div key={extraIndex} className="flex justify-between ml-4 text-gray-500">
                                <span>• {extra.name}</span>
                                <span>{formatCurrency(extra.cost)} × {window.quantity}</span>
                              </div>
                            ))}
                          </div>
                        )}
                        ...
                        {window.finishes.length > 0 && (
                          <div>
                            <div className="flex justify-between font-medium">
                              ...
                              <span className="text-gray-900">
                                {formatCurrency(window.finishesTotal * window.quantity)}
                              </span>
                            </div>
                            {window.finishes.map((finish, finishIndex) => (
                              <div key={finishIndex} className="flex justify-between ml-4 text-gray-500">
                                <span>• {finish.name}</span>
                                <span>{formatCurrency(finish.cost)} × {window.quantity}</span>
                              </div>
                            ))}
                          </div>
                        )}
            ...
            <div className="px-6 py-4">
              <div className="space-y-3">
                <div className="flex justify-between">
                  ...
                  <span className="text-gray-900">{formatCurrency(estimate.breakdown.subtotal)}</span>
                </div>
                
                {estimate.breakdown.discountAmount && (
                  <div className="flex justify-between text-red-600">
                    <span>Discount ({estimate.breakdown.discountPercent}%):</span>
                    <span>-{formatCurrency(estimate.breakdown.discountAmount)}</span>
                  </div>
                )}
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-gray-900">{formatCurrency(estimate.breakdown.vat)}</span>
                </div>
                
                <div className="flex justify-between text-xl font-bold border-t border-gray-200 pt-3">
                  <span className="text-gray-900">Total:</span>
                  <span className="text-gray-900">{formatCurrency(estimate.breakdown.total)}</span>
                </div>
              </div>
            </div>
          </div>
              ...
              <div className="flex">
                ...
                <div className="ml-3">
                  ...
                  <div className="mt-2 text-sm text-blue-700">
                    <p>
                      This estimate is ready to be synced to the server.
                      PDF generation will be available after syncing.
                    </p>
                  </div>
                </div>
              </div>
